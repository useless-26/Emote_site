...
FIREBASE_URL = 'https://bijayxahsg72-default-rtdb.firebaseio.com'
...
def init_firebase():
    ...
    try:
        ...
        print('[FB] Firebase initialized OK')
        return True
...
DATABASE = {k: v for k, v in DEFAULT_DATA.items()}

def fb_read(path):
    """Read a path from Firebase, returns None on error."""
    try:
        ref = firebase_db.reference(path)
        return ref.get()
    except Exception as e:
        print(f'[FB] Read error {path}: {e}')
        return None
...
def fb_write(path, data):
    ...
    try:
        ...
        print(f'[FB] Write error {path}: {e}')
        return False

def load_database():
    """Load all data from Firebase into in-memory DATABASE cache."""
    global DATABASE
    if not init_firebase():
        print('[DB] Firebase not available, using defaults')
        return
    try:
        data = fb_read('/')
        if data:
            ...
            # Convert Firebase object-style emotes/servers/categories back to lists
            for list_key in ['emotes', 'servers', 'categories']:
                if isinstance(DATABASE.get(list_key), dict):
                    DATABASE[list_key] = list(DATABASE[list_key].values())
            print(f'[DB] Loaded — emotes:{len(DATABASE.get("emotes",[]))} servers:{len(DATABASE.get("servers",[]))} cats:{len(DATABASE.get("categories",[]))}')
        else:
            print('[DB] No data in Firebase yet — saving defaults')
            save_all()
    except Exception as e:
        print(f'[DB] load error: {e}')

def save_all():
    """Save entire DATABASE to Firebase."""
    if not init_firebase():
        return False
    return fb_write('/', DATABASE)

def save_section(key):
    """Save one top-level key (emotes, servers, etc.) to Firebase."""
    if not init_firebase():
        return False
    ok = fb_write(f'/{key}', DATABASE[key])
    if ok:
        print(f'[FB] Saved /{key} — {len(DATABASE[key]) if isinstance(DATABASE[key], list) else "..."} items')
    return ok
...
def login():
    ...
    stored = DATABASE.get('users', {}).get('admin', '')
    if not stored:
...
def get_data():
    return jsonify({
        ...
        'emotes': DATABASE.get('emotes', []),
        'settings': DATABASE.get('settings', DEFAULT_DATA['settings'])
    })

@app.route('/api/admin/login', methods=['POST'])
def admin_login():
    data = request.json or {}
    if data.get('email') == 'xcy@example.com' and data.get('password') == 'Bijayvip':
        return jsonify({'success': True})
...
def manage_servers():
    if request.method == 'GET':
        return jsonify({'servers': DATABASE.get('servers', [])})
        ...
        try:
            ...
            for i, s in enumerate(DATABASE['servers']):
                if str(s['id']) == server_id:
                    DATABASE['servers'][i] = {'id': server_id, 'name': data.get('name','').strip(), 'baseUrl': data.get('baseUrl','').strip().rstrip('/'), 'region': data.get('region',''), 'order': int(data.get('order') or 0)}
                    save_section('servers')
                    return jsonify({'success': True})
        ...
        try:
            ...
            DATABASE['servers'] = [s for s in DATABASE['servers'] if str(s['id']) != sid]
            save_section('servers')
            return jsonify({'success': True})
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)})
...
def manage_categories():
    if request.method == 'GET':
        return jsonify({'categories': DATABASE.get('categories', [])})
    elif request.method == 'POST':
        try:
            data = request.json or {}
            name = data.get('name', '').strip()
            category = {'id': data.get('id') or name.upper().replace(' ', '_'), 'name': name, 'icon': data.get('icon', ''), 'order': int(data.get('order') or 0)}
            DATABASE['categories'].append(category)
            save_section('categories')
        ...
        try:
            ...
            for i, c in enumerate(DATABASE['categories']):
                if str(c['id']) == cid:
                    DATABASE['categories'][i] = {'id': cid, 'name': data.get('name','').strip(), 'icon': data.get('icon',''), 'order': int(data.get('order') or 0)}
                    save_section('categories')
                    return jsonify({'success': True})
            return jsonify({'success': False, 'error': 'Not found'})
        except Exception as e:
        ...
        try:
            ...
            DATABASE['categories'] = [c for c in DATABASE['categories'] if str(c['id']) != cid]
            save_section('categories')
            return jsonify({'success': True})
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)})
...
def manage_emotes():
    if request.method == 'GET':
        return jsonify({'emotes': DATABASE.get('emotes', [])})
    elif request.method == 'POST':
        try:
            data = request.json or {}
            ...
            if not image_url or not category:
                return jsonify({'success': False, 'error': 'imageUrl and category required'})
            emote = {
                'id': str(int(time.time() * 1000)),
                'imageUrl': image_url,
                'category': category,
            ...
            save_section('emotes')
            print(f'[EMOTE] Added: {emote["emoteId"]} cat={category} total={len(DATABASE["emotes"])}')
        ...
        try:
            ...
            image_url = data.get('imageUrl', '').strip()
            category = data.get('category', '').strip()
            for i, em in enumerate(DATABASE['emotes']):
                if str(em['id']) == eid:
                    DATABASE['emotes'][i] = {'id': eid, 'imageUrl': image_url, 'category': category, 'emoteId': data.get('emoteId') or image_url.split('/')[-1].split('.')[0]}
                    save_section('emotes')
                    return jsonify({'success': True})
            return jsonify({'success': False, 'error': f'Emote {eid} not found'})
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)})
    elif request.method == 'DELETE':
        try:
            eid = str(request.args.get('id', ''))
            before = len(DATABASE['emotes'])
            DATABASE['emotes'] = [e for e in DATABASE['emotes'] if str(e['id']) != eid]
            save_section('emotes')
            print(f'[EMOTE] Deleted {eid}, removed {before - len(DATABASE["emotes"])}')
            return jsonify({'success': True})
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)})
...
def manage_settings():
    if request.method == 'GET':
        return jsonify(DATABASE.get('settings', DEFAULT_DATA['settings']))
    elif request.method == 'POST':
        try:
            data = request.json or {}
            stype = data.get('type')
            if stype == 'maintenance':
                DATABASE['settings']['maintenance'] = {'enabled': bool(data.get('enabled', False)), 'message': data.get('message', '')}
            elif stype == 'footerLinks':
                ...
                if pw:
                    DATABASE['users']['admin'] = hash_password(pw)
                    save_section('users')
...
def send_emote():
    ...
    uids = {f'uid{i}': request.args.get(f'uid{i}') for i in range(1,6) if request.args.get(f'uid{i}')}
    if not server or not tc or not emote_id:
        return jsonify({'success': False, 'error': 'Missing required parameters'})
    params = {'tc': tc, 'emote_id': emote_id, **uids}
    target_url = f"{server}/join?" + '&'.join(f'{k}={v}' for k,v in params.items())
    try:
        r = requests.get(target_url, timeout=10)
        return jsonify({'success': True, 'status': r.status_code, 'message': 'Emote sent', 'data': r.text})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
